...
# Setup
	...
	dates_letnum = 2 #the column/row which has the dates in it
	entries_to_exclude = ['X', 'OFF', np.nan]
...
# Main
	...
	names_col = df['A'].tolist()
	for i in range(0, len(names_col)):
		if i not in [0, 1]:
			if 'Juniors total' in names_col[i]:
				nrows = i
				break
...
if __name__ == '__main__':
	print(os.getcwd())
	print(convert('../../../user_input/input_brighton_gastro.xlsx', {
		'row_number': '10',
		'col_start': 'dz',
		'col_end': 'Io',
